...
object ChartMath {
    ...
    /** Normalize non-negative bar values against the largest visible value. */
    fun barFractions(values: List<Float>): List<Float> {
        val safeValues = values.map { value ->
            value.takeIf { it.isFinite() }?.coerceAtLeast(0f) ?: 0f
        }
        val max = safeValues.maxOrNull() ?: return emptyList()
        if (max == 0f) return List(safeValues.size) { 0f }
        return safeValues.map { it / max }
    }
}
